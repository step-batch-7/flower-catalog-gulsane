...
const saveComment = function(req, res) {
  ...
  fs.writeFileSync('./data/userComments.json', `${JSON.stringify(comments)}`);
...
const serveGuestBook = function(req, res) {
  ...
  comments = comments.map(comment => {
    return new Comment(comment.name, comment.commentMsg, comment.time);
  });

  const commentTable = comments.map(comment => comment.toHTML());
  let content = fs.readFileSync('./public/guestBook.html', 'utf8');
  content = content.replace('__comments__', commentTable.join(''));
